"""FastAPI application for GMNet web service."""
...
CHECKPOINTS_DIR = os.environ.get("GMNET_CHECKPOINTS_DIR", "/app/checkpoints")
...
async def lifespan(app: FastAPI):
    ...
    from .inference import GMNetPipeline
    logger.info("Loading GMNet pipeline...")

    app.state.pipeline = GMNetPipeline(CHECKPOINTS_DIR)
    ...
    logger.info("GMNet service ready.")
    yield
...
app = FastAPI(title="GMNet Service", lifespan=lifespan)
...
async def health():
    return {
        "status": "ok",
        "device": str(app.state.pipeline.device),
        "available_models": app.state.pipeline.available_models,
        "current_model": app.state.pipeline._current_model,
...
async def generate(job_id: str, req: GenerateRequest):
    ...
    if req.model not in app.state.pipeline.available_models:
        raise HTTPException(503, f"Model '{req.model}' not available")
