...
// Stored input linear metrics for comparison with HDR output
let inputLinearMetrics = null; // { mean, peak, dr }
...
// --- Init: populate model select from health endpoint ---
(async function initModels() {
    try {
        ...
        for (const m of data.available_models) {
            ...
            opt.value = m;
            opt.textContent = m;
            if (m === 'realworld') opt.selected = true;
            select.appendChild(opt);
        }
    } catch (e) {
        console.warn('Could not fetch available models:', e);
        const select = $('#model-select');
        select.innerHTML = '<option value="realworld" selected>realworld</option><option value="synthetic">synthetic</option>';
    }
})();
...
// --- Slider labels ---
$('#peak-slider').addEventListener('input', (e) => {
    $('#peak-val').textContent = parseFloat(e.target.value).toFixed(1);
});
...
$('#generate-btn').addEventListener('click', async () => {
    ...
    const peak = parseFloat($('#peak-slider').value);
    const scale = parseInt($('#scale-select').value);
    hide($('#error-section'));
    hide($('#result-section'));
    hdrData = null;
    ...
    try {
        const resp = await fetch(`/api/generate/${currentJobId}`, {
            ...
            body: JSON.stringify({ model, peak, scale }),
        });
...
function displayResult(data) {
    ...
    if (inM) {
        ...
        // Contrast ratio (P99.9 / P0.1) — this is the meaningful comparison
